...
const updatePlayerRatings = async (lobbyState) => {
  ...
  const r1 =
    faction1.reduce((total, player) => total + player.rating, 0) /
    faction1.length;
  const r2 =
    faction2.reduce((total, player) => total + player.rating, 0) /
    faction2.length;
...
const getHeroNameFromId = (id) =>
  heroes[id] ? heroes[id].localized_name : "Unknown";
...
const createMatchEndMessageEmbed = async (matchId) => {
  ...
  const captain1Data = odotaData.players.find(
    (player) =>
      player.account_id.toString() === convertor.to32(captain1.steamId64)
  );
  const captain2Data = odotaData.players.find(
    (player) =>
      player.account_id.toString() === convertor.to32(captain2.steamId64)
  );
  const radiantCaptainPlayer = captain1Data.isRadiant
    ? captain1Data
    : captain2Data;
  ...
  const radiantDetails = odotaData.players
    .filter((player) => player.isRadiant)
    .map(createMatchPlayerDetails)
    .join("\t\n\n");
  const direDetails = odotaData.players
    .filter((player) => !player.isRadiant)
    .map(createMatchPlayerDetails)
    .join("\t\n\n");
  ...
  const goldLead =
    odotaData.radiant_gold_adv[odotaData.radiant_gold_adv.length - 1];
